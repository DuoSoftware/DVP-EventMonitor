...
var notifyGatewayStatus = function (reqId, status, evtObj) {
  mongoAccessor.getSuperUsers(function (err, usrData) {
    if (usrData && usrData.length > 0) {
      ...
      extApiAccess.SendNotificationBroadcast(
        reqId,
        status,
        evtObj["Gateway"],
        nsObj,
          ...
          if (superuser.username) {
            if (superuser.email && superuser.email.contact) {
              sendMailSMS(
                ...
                superuser.company,
                superuser.tenant,
                superuser.email.contact,
                evtObj["Gateway"] +
                  " Gateway is DOWN at " +
                  evtObj["Event-Date-GMT"],
                null
              );
            }
      ...
      logger.debug(
        "[DVP-EventMonitor.handler] - [%s] - No superusers to broadcast message",
        reqId
      );
    }
  });
...
var eventHandler = function (reqId, evtObj) {
  ...
  var chanCountCompany =
    "DVP_CHANNEL_COUNT_COMPANY:" + tenantId + ":" + companyId;
  ...
  if (
    evtType === "CHANNEL_BRIDGE" ||
    evtType === "CHANNEL_CALLSTATE" ||
    evtType === "CHANNEL_CREATE" ||
    evtType === "CHANNEL_STATE" ||
    evtType === "CHANNEL_ANSWER"
  ) {
    if (dvpCallDirection) {
      redisClient.hset(
        uniqueId,
  ...
  if (
    ...
    evtType === "CHANNEL_ANSWER" ||
    evtType === "ARDS_EVENT" ||
    evtType === "CHANNEL_HOLD" ||
    evtType === "CHANNEL_UNHOLD" ||
    evtType === "CHANNEL_UNBRIDGE" ||
    evtType === "CHANNEL_DESTROY" ||
    evtType === "CHANNEL_HANGUP"
  ) {
    logger.debug(
      "[DVP-EventMonitor.handler] - [%s] - Event Data - EVENT_TYPE : " +
        evtType +
        ", CHANNEL_STATE : " +
        evtObj["Channel-State"] +
        ", SESSION_ID : " +
        uniqueId +
        ", CALLER_UUID : " +
        evtObj["Caller-Unique-ID"] +
        "SWITCH HOST NAME : " +
        freeswitchHostname,
      reqId
    );
  ...
  if (
    ...
    if (dialerAgentEvtPub) {
      ...
      logger.debug(
        "[DVP-EventMonitor.handler] - [%s] - REDIS PUBLISH DIALER AGENT CHANNEL : %s, DATA: %s",
        reqId,
        dialerAgentEvtPub,
        jsonStr
      );
  ...
  switch (evtType) {
    case "ARDS_EVENT":
      ...
      if (dvpCustPubId) {
        ...
        logger.debug(
          "[DVP-EventMonitor.handler] - [%s] - REDIS PUBLISH CUSTOM: %s",
          reqId,
          jsonStr
        );
      } else {
    ...
    case "CHANNEL_BRIDGE":
      ...
      if (
        (opCat === "ATT_XFER_USER" || opCat === "ATT_XFER_GATEWAY") &&
        tenantId &&
        companyId &&
        resId &&
        varArdsClientUuid &&
        otherLegDir === "outbound"
      ) {
        dashboardEvtHandler.PublishDashboardMessage(
          tenantId,
          companyId,
          bUnit,
          "CALLSERVER",
      ...
      if (dvpCallDirection) {
        ...
        if (
          opCat === "ATT_XFER_GATEWAY" &&
          otherLegContext === "PBXFeatures" &&
          otherLegCalleeIdNumber &&
          otherLegCallerIdNumber &&
          dvpCallDirection === "outbound" &&
          operator
        ) {
          extApiAccess.BillCall(
            reqId,
            uniqueId,
            otherLegCallerIdNumber,
            otherLegCalleeIdNumber,
        ...
        var callCountCompanyDir =
          "DVP_CALL_COUNT_COMPANY_DIR:" +
          tenantId +
          ":" +
          companyId +
          ":" +
        ...
        if (callMonitorOtherLeg) {
          ...
          if (evtObj["variable_fifo_bridge_uuid"]) {
            redisClient.hset(
              uniqueId,
              "Other-Leg-Unique-ID",
              evtObj["variable_fifo_bridge_uuid"],
              redisMessageHandler
            );
            redisClient.hset(
              uniqueId,
    ...
    case "CHANNEL_CREATE":
      ...
      if (
        ...
        redisClient.get(
          "SIPUSER_RESOURCE_MAP:" + tenantId + ":" + companyId + ":" + digits,
          function (err, objString) {
            var obj = JSON.parse(objString);

            if (obj && obj.Context) {
              redisClient.get(
                "SIPUSER_RESOURCE_MAP:" +
                  tenantId +
                  ":" +
                  companyId +
                  ":" +
                ...
                function (err, objString) {
                  ...
                  extApiAccess.SendNotificationInitiate(
                    reqId,
                    "agent_found",
                    uniqueId,
                    nsObj,
                  ...
                  logger.debug(
                    "[DVP-EventMonitor.handler] - [%s] - SEND NOTIFICATION - AGENT FOUND - Message : ",
                    reqId,
                    nsObj.Message
                  );
                }
            ...
            function (err, objString) {
              ...
              if (objTmp && objTmp.Context) {
                ...
                var nsObj = {
                  ...
                  Message:
                    "agent_found|" +
                    evtObj["variable_origination_caller_id_number"] +
                    "|OUTBOUND|" +
                    objTmp.Issuer +
                    "|" +
                    evtObj["Caller-ANI"] +
        ...
        if (actionCat === "DIALER" && callLegType === "AGENT") {
          redisClient.get(
            "SIPUSER_RESOURCE_MAP:" +
              ...
              companyId +
              ":" +
              evtObj["Caller-Destination-Number"],
            function (err, objString) {
              var obj = JSON.parse(objString);

              if (obj && obj.Context) {
                logger.debug(
                  "[DVP-EventMonitor.handler] - [%s] - OUTBOUND CHANNEL - SENDING",
                  reqId
                );
                ...
                extApiAccess.CreateEngagement(
                  ...
                  "outbound",
                  evtObj["Caller-Destination-Number"],
                  evtObj["Other-Leg-Caller-ID-Number"],
                  companyId,
                  tenantId
                );
                ...
                nsObj.Message =
                  ...
                  obj.Issuer +
                  "|" +
                  evtObj["variable_ards_skill_display"] +
                  "|outbound|call|undefined" +
                  otherLegUniqueId +
                  "|DIALER";
                ...
                logger.debug(
                  "[DVP-EventMonitor.handler] - [%s] - OUTBOUND CHANNEL - CONTEXT NOT FOUND",
                  reqId
                );
              }
            }
            ...
            function (err, objString) {
              ...
              if (obj && obj.Context) {
                      ...
                      if (objTmp && objTmp.Context) {
                        ...
                        var nsObj = {
                          ...
                          Message:
                            ...
                            "|" +
                            obj.Issuer +
                            "|OUTBOUND|outbound|call|undefined|" +
                            uniqueId,
                        };
                ...
                if (opCat === "GATEWAY" || opCat === "PRIVATE_USER") {
                  ...
                  ardsHandler.SendResourceStatus(
                    reqId,
                    otherLegUniqueId,
                    obj.CompanyId,
                    obj.TenantId,
      ...
      if (
        ...
        companyId &&
        tenantId &&
        ((opCat === "PRIVATE_USER" && dvpCallDirection === "outbound") ||
          opCat === "ATT_XFER_USER")
      ) {
        redisClient.get(
          "SIPUSER_RESOURCE_MAP:" +
    ...
    case "CHANNEL_ANSWER":
      ...
      logger.debug(
        "[DVP-EventMonitor.handler] - [%s] - CHANNEL ANSWER ARDS DATA - EVENT_TYPE : " +
          evtType +
          ", SESSION_ID : " +
          uniqueId +
          "SWITCH NAME : " +
          switchName +
          "ards_client_uuid : %s, companyid : %s, tenantid : %s, ards_resource_id : %s, ards_servertype : %s, ards_requesttype : %s",
        reqId,
        ardsClientUuid,
        ardsCompany,
        ardsTenant,
        ...
        redisClient.get(
          ...
          function (err, objString) {
            ...
            if (obj && obj.Context) {
              ...
              logger.debug(
                "[DVP-EventMonitor.handler] - [%s] - SEND NOTIFICATION - AGENT CONNECTED - Message : ",
                reqId,
                nsObj.Message
              );
            } else {
      ...
      if (opCat === "ATT_XFER_USER") {
        ...
        if (
          ...
          if (evtObj["variable_OriginalUuidARDS"]) {
            ardsHandler.SendResourceStatus(
              reqId,
              evtObj["variable_OriginalUuidARDS"],
              ardsCompany,
              ardsTenant,
              ardsServerType,
              ardsReqType,
    ...
    case "HEARTBEAT":
      ...
      if (switchName) {
        redisClient.set(
          switchName + "#DVP_CS_INSTANCE_INFO",
    ...
    case "CHANNEL_HANGUP":
      ...
      if (
        ...
        ardsTenant &&
        opCat &&
        (evtObj["variable_user_id"] || evtObj["variable_dialed_user"]) &&
        actionCat != "DIALER"
      ) {
      ...
      if (
        evtObj["variable_dvp_trans_caller"] &&
        evtObj["variable_dvp_trans_party"]
      ) {
        redisClient.get(
          "SIPUSER_RESOURCE_MAP:" +
            ardsTenant +
          ...
          function (err, objString) {
            ...
            if (obj && obj.Context) {
              ...
              logger.debug(
                "[DVP-EventMonitor.handler] - [%s] - SEND NOTIFICATION - AGENT TRANSFER ENDED - Message : ",
                reqId,
                nsObj.Message
              );
            }
      ...
      if (ardsClientUuid) {
              ...
              if (obj && obj.Context) {
                var nsObj = {
                  ...
                  Message:
                    ...
                    uniqueId +
                    "|OUTBOUND|" +
                    evtObj["Caller-Caller-ID-Number"] +
                    "|" +
                    evtObj["Caller-Caller-ID-Number"] +
                    "|" +
                    obj.Issuer +
                ...
                logger.debug(
                  "[DVP-EventMonitor.handler] - [%s] - SEND NOTIFICATION - AGENT DISCONNECTED - Message : ",
                  reqId,
                  nsObj.Message
                );
              }
        ...
        evtObj["variable_DVP_CLICKTOCALL"] === "C2C" &&
        direction === "outbound" &&
        companyId &&
        tenantId &&
        opCat === "GATEWAY" &&
      ...
      if (
        opCat === "ATT_XFER_GATEWAY" &&
        direction === "outbound" &&
        callerContext === "PBXFeatures" &&
        dvpCallDirection === "outbound"
      ) {
        extApiAccess.BillEndCall(
          reqId,
          varOrigLegUuid,
          varCallerIdNum,
          callerDestNum,
    ...
    case "CHANNEL_DESTROY":
      ...
      if (companyId && tenantId) {
        ...
        logger.debug(
          "=========================== REMOVE FROM SET - COMPANY NOT SET : [%s] ==========================",
          channelSetName,
          uniqueId
        );
        ...
        redisClient.get("CHANNELMAP:" + uniqueId, function (err, mapObj) {
          if (mapObj) {
            ...
            logger.debug(
              "=========================== REMOVE FROM SET WITH FAILSAFE: [%s] - [%s] ==========================",
              mapObj,
              uniqueId
            );
          }
    ...
    case "PRESENCE_IN":
      ...
      if (uriSplit && uriSplit.length == 2) {
        if (userStatus === "Busy" || userStatus === "Available") {
          ...
          redisClient.set(
            "SIPPRESENCE:" + uriSplit[1] + ":" + uriSplit[0],
            JSON.stringify(presObj),
            redisMessageHandler
          );
      ...
      redisClient.get(
        ...
        function (err, objString) {
          ...
          if (obj && obj.Context) {
            ...
            logger.debug(
              "[DVP-EventMonitor.handler] - [%s] - SEND NOTIFICATION - AGENT TRANSFER TRYING - Message : ",
              reqId,
              nsObj.Message
            );
          }
    ...
    case "CUSTOM":
      if (evtObj["Event-Subclass"]) {
        ...
        if (subClass == "conference::maintenance") {
          ...
          if (evtObj["Action"]) {
            ...
            switch (action) {
                    ...
                    if (confInfo) {
                      var nsObj = {
                        ...
                        Message:
                          "Conference member " +
                          userName +
                          " state is now listening",
                      };

                      extApiAccess.SendNotificationByKey(
                        reqId,
                        "conference_create",
                        conferenceID,
                        "CONFERENCE:" + conferenceName,
              ...
              case "conference-destroy":
                ...
                redisClient.del(
                  "CONFERENCE-COUNT:" + conferenceName,
                  redisMessageHandler
                );
                redisClient.del(
                  "CONFERENCE-MEMBERS:" + conferenceName,
                  userName,
                  redisMessageHandler
                );
                redisClient.del(
                  ...
                  dbOp.GetConferenceRoom(function (err, confInfo) {
                    if (confInfo) {
                      extApiAccess.SendNotificationByKey(
                        ...
                        conferenceID,
                        "CONFERENCE:" + conferenceName,
                        "Conference room " + conferenceName + " closed",
                        conferenceID,
                        confInfo.CompanyId,
                        confInfo.TenantId
                      );
                  ...
                  dbOp.GetConferenceRoom(function (err, confInfo) {
                    if (confInfo) {
                      extApiAccess.SendNotificationByKey(
                        ...
                        conferenceID,
                        "CONFERENCE:" + conferenceName,
                        "Conference member " + userName + " joined",
                        conferenceID,
                        confInfo.CompanyId,
                        confInfo.TenantId
                      );
                  ...
                  dbOp.GetConferenceRoom(function (err, confInfo) {
                    if (confInfo) {
                      extApiAccess.SendNotificationByKey(
                        ...
                        conferenceID,
                        "CONFERENCE:" + conferenceName,
                        "Conference member " + userName + " left",
                        conferenceID,
                        confInfo.CompanyId,
                        confInfo.TenantId
                      );
                ...
                if (conferenceName) {
                  dbOp.GetConferenceRoom(function (err, confInfo) {
                    if (confInfo) {
                      extApiAccess.SendNotificationByKey(
                        reqId,
                        "conference_member_status",
                        conferenceID,
                        "CONFERENCE:" + conferenceName,
                        "Conference member " +
                          userName +
                          " state is now talking",
                        conferenceID,
                        confInfo.CompanyId,
                        confInfo.TenantId
                      );
          ...
          logger.debug(
            "[DVP-EventMonitor.handler] - [%s] - CUSTOM EVENT - EVENT-SUBCLASS : %s, CC-ACTION : %s, CC-AGENT : %s, AGENT-STATUS : %s, AGENT-STATE : %s",
            reqId,
            subClass,
            action,
            agent,
            ...
            if (action == "agent-state-change") {
              ...
              if (agentStatus) {
                ...
                if (
                  ...
                  extApiAccess.GetModCallCenterAgentCount(
                    reqId,
                    campId,
                    function (err, fsResp) {
                      if (fsResp) {
                        ...
                        logger.debug(
                          "[DVP-EventMonitor.handler] - [%s] - ==================== SET LIMIT ====================",
                          reqId
                        );
                        extApiAccess.SetMaxChanLimit(
                          reqId,
                          campId,
                          repStr,
                          "",
          ...
          var key = util.format(
            "Park-%s-%s",
            evtObj["Valet-Lot-Name"],
            evtObj["Valet-Extension"]
          );
          ...
          switch (subClass) {
            case "sofia::register":
              ...
              dbOp.getUserDetailsByDomain(username, realm, function (err, usr) {
                ...
                if (usr && usr.GuRefId) {
                  ...
                  mongoAccessor.getUserAccountData(
                    usr.CompanyId,
                    usr.TenantId,
                    usr.GuRefId,
            ...
            case "sofia::gateway_add":
              redisClient.set(
                "TRUNK_AVAILABILITY:" + evtObj["Gateway"],
                JSON.stringify(evtObj),
                redisMessageHandler
              );
          ...
          if (action === "agent-rejected") {
            ...
            var eventParam = util.format(
              "The call is rejected by %s",
              evResource
            );
            ...
            var evResource = evtObj["ARDS-Resource-Name"];
            var eventParam = util.format(
              "%s is selected to route the request",
              evResource
            );
            evtData.EventParams = eventParam;
            /*var jsonStr = JSON.stringify(evtData);
...
var handler = function (event, header, body) {
  ...
  try {
    if (header) {
      ...
      evtObj["variable_CustomCompanyStr"] = event.getHeader(
        "variable_CustomCompanyStr"
      );
      evtObj["variable_DVP_CUSTOM_PUBID"] = event.getHeader(
        "variable_DVP_CUSTOM_PUBID"
      );
      ...
      evtObj["variable_business_unit"] = event.getHeader(
        "variable_business_unit"
      );
      evtObj["variable_tenantid"] = event.getHeader("variable_tenantid");
      evtObj["variable_veeryoperator"] = event.getHeader(
        "variable_veeryoperator"
      );
      evtObj["Event-Date-Timestamp"] = event.getHeader("Event-Date-Timestamp");
      evtObj["FreeSWITCH-Switchname"] = event.getHeader(
        "FreeSWITCH-Switchname"
      );
      evtObj["Caller-Destination-Number"] = event.getHeader(
        "Caller-Destination-Number"
      );
      ...
      evtObj["variable_application_type"] = event.getHeader(
        "variable_application_type"
      );
      evtObj["variable_application_position"] = event.getHeader(
        "variable_application_position"
      );
      evtObj["Caller-Caller-ID-Number"] = event.getHeader(
        "Caller-Caller-ID-Number"
      );
      evtObj["Caller-Caller-ID-Name"] = event.getHeader(
        "Caller-Caller-ID-Name"
      );
      evtObj["Call-Direction"] = event.getHeader("Call-Direction");
      evtObj["variable_DVP_CALL_DIRECTION"] = event.getHeader(
        "variable_DVP_CALL_DIRECTION"
      );
      evtObj["variable_DVP_CALLMONITOR_OTHER_LEG"] = event.getHeader(
        "variable_DVP_CALLMONITOR_OTHER_LEG"
      );
      evtObj["Other-Leg-Unique-ID"] = event.getHeader("Other-Leg-Unique-ID");
      evtObj["Caller-Orig-Caller-ID-Name"] = event.getHeader(
        "Caller-Orig-Caller-ID-Name"
      );
      evtObj["Caller-Orig-Caller-ID-Number"] = event.getHeader(
        "Caller-Orig-Caller-ID-Number"
      );
      evtObj["variable_DVP_OPERATION_CAT"] = event.getHeader(
        "variable_DVP_OPERATION_CAT"
      );
      evtObj["variable_DVP_ACTION_CAT"] = event.getHeader(
        "variable_DVP_ACTION_CAT"
      );
      evtObj["variable_ARDS-Resource-Id"] = event.getHeader(
        "variable_ARDS-Resource-Id"
      );
      ...
      evtObj["variable_ards_resource_id"] = event.getHeader(
        "variable_ards_resource_id"
      );
      evtObj["variable_ards_skill_display"] = event.getHeader(
        "variable_ards_skill_display"
      );
      evtObj["Caller-Channel-Bridged-Time"] = event.getHeader(
        "Caller-Channel-Bridged-Time"
      );
      evtObj["variable_ards_client_uuid"] = event.getHeader(
        "variable_ards_client_uuid"
      );
      evtObj["Other-Leg-Direction"] = event.getHeader("Other-Leg-Direction");
      evtObj["Other-Leg-Caller-ID-Number"] = event.getHeader(
        "Other-Leg-Caller-ID-Number"
      );
      evtObj["Other-Leg-Callee-ID-Number"] = event.getHeader(
        "Other-Leg-Callee-ID-Number"
      );
      evtObj["Other-Leg-Context"] = event.getHeader("Other-Leg-Context");
      evtObj["variable_fifo_bridge_uuid"] = event.getHeader(
        "variable_fifo_bridge_uuid"
      );
      ...
      evtObj["variable_sip_gateway_name"] = event.getHeader(
        "variable_sip_gateway_name"
      );
      evtObj["variable_loopback_app"] = event.getHeader(
        "variable_loopback_app"
      );
      evtObj["variable_sip_auth_realm"] = event.getHeader(
        "variable_sip_auth_realm"
      );
      evtObj["Channel-Call-UUID"] = event.getHeader("Channel-Call-UUID");
      evtObj["variable_ards_servertype"] = event.getHeader(
        "variable_ards_servertype"
      );
      evtObj["variable_ards_requesttype"] = event.getHeader(
        "variable_ards_requesttype"
      );
      ...
      evtObj["Session-Since-Startup"] = event.getHeader(
        "Session-Since-Startup"
      );
      evtObj["Idle-CPU"] = event.getHeader("Idle-CPU");
      evtObj["variable_origination_caller_id_number"] = event.getHeader(
        "variable_origination_caller_id_number"
      );
      evtObj["variable_originating_leg_uuid"] = event.getHeader(
    ...
    logger.error(
      "[DVP-EventMonitor.handler] - [%s] - Exception on handler ",
      reqId,
      e
    );
  }
                ...
                try {
                  conn.subscribe(
                    ...
                    function () {
                      ...
                      logger.info(
                        "[DVP-EventMonitor.AppStart] - Subscribed to FS Events....."
                      );
                      flag = false;
                      CreateESLWithTimeout();
                    }
...
if (evtConsumeType) {
  if (evtConsumeType === "esl") {
    ...
    connection.on("connect", function () {
      ...
      logger.debug(
        "[DVP-EventMonitor.handler] - [%s] - AMQP Connection CONNECTED"
      );
    });
