...
loggerCust.add(winston.transports.File, {
  filename: "/logs/ards_logger.log",
  level: level,
  maxsize: 1242880,
  maxFiles: 10,
});

var SendResourceStatus = function (
  reqId,
  ardsClientUuid,
  ardsCompany,
  ardsTenant,
  ...
  try {
    if (ardsClientUuid && ardsCompany && ardsTenant && ardsResourceId) {
      logger.debug(
        "[DVP-EventMonitor.SendResourceStatus] - [%s] -  Creating PUT Message",
        reqId
      );
      ...
      if (ardsIp && ardsPort && ardsVersion) {
        ...
        if (config.dynamicPort || validator.isIP(ardsIp)) {
          httpUrl = util.format(
        ...
        httpReq.put(options, function (error, response, body) {
          if (
            ...
            logger.error(
              "[DVP-EventMonitor.SendResourceStatus] - [%s] - Set Resource Status Fail - Response : [%s]",
              reqId,
        ...
        logger.error(
          "[DVP-EventMonitor.SendResourceStatus] - [%s] - ARDS Endpoints not defined",
          reqId,
    ...
    logger.error(
      "[DVP-EventMonitor.SendResourceStatus] - [%s] - Exception Occurred",
      reqId,
      ex
    );
  }
